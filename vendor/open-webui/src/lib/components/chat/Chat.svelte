<script lang="ts">
	...
	const setDefaults = async () => {
		if (($config?.features?.enable_tools ?? false) && !$tools) {
			tools.set(await getTools(localStorage.token));
		}
		if (($config?.features?.enable_tools ?? false) && !$functions) {
	...
	const sendMessageSocket = async (model, _messages, _history, responseMessageId, _chatId) => {
		...
		if (res) {
			if (res.error) {
				...
				// Superhuman backend can return direct OpenAI-style JSON without task streaming.
